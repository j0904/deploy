...
resource "null_resource" "bigt2Webserver1HTTPD" {
  depends_on = [oci_core_instance.bigt2Webserver1]
  triggers = {
  ...
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "ubuntu"
      host        = data.oci_core_vnic.bigt2Webserver1_VNIC1.public_ip_address
      agent       = true
      timeout     = "10m"
    }
    inline = [
